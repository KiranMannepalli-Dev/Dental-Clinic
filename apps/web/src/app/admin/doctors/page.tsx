...
import { 
  ...
  Clock, Save, ShieldAlert, Award, FileText, CheckSquare
} from "lucide-react";
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-slate-500 text-xs uppercase tracking-wider">
                <th className="p-4 font-medium">Specialist Details</th>
                <th className="p-4 font-medium">Qualifications</th>
                <th className="p-4 font-medium">Fee & Experience</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-slate-500">Loading specialists...</td>
                </tr>
              ) : doctors.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-slate-500">No specialists registered in DB.</td>
                </tr>
              ) : (
                doctors.map((doc) => (
                  <tr key={doc.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-md bg-blue-550/10 text-blue-600 flex items-center justify-center font-bold text-sm shrink-0 border border-blue-100 uppercase">
                          {doc.firstName[0]}{doc.lastName[0]}
                        </div>
                        <div>
                          <p className="font-semibold text-slate-900 text-sm">Dr. {doc.firstName} {doc.lastName}</p>
                          <p className="text-xs text-slate-500 mt-0.5">{doc.email} {doc.phone && `• ${doc.phone}`}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Qualifications */}
                    <td className="p-4">
                      <p className="text-sm font-medium text-slate-800">{doc.specialization}</p>
                      <p className="text-xs text-slate-500 mt-0.5">{doc.qualification}</p>
                    </td>

                    {/* Fee & Experience */}
                    <td className="p-4">
                      <p className="text-sm text-slate-900 font-semibold">₹{parseFloat(doc.consultationFee).toLocaleString()}</p>
                      <p className="text-xs text-slate-500 mt-0.5">{doc.experience} years experience</p>
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex flex-col gap-2">
                        ...
                        <label className="inline-flex items-center gap-2 cursor-pointer select-none">
                          ...
                          />
                          <span className={`text-xs font-semibold ${doc.isActive ? 'text-green-700' : 'text-slate-400'}`}>
                        ...
                        <label className="inline-flex items-center gap-2 cursor-pointer select-none">
                          ...
                          />
                          <span className={`text-xs font-semibold ${doc.isFeatured ? 'text-yellow-700 font-semibold' : 'text-slate-400'}`}>
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm font-semibold text-slate-800">{doc.rating.toFixed(1)}</span>
                        <span className="text-xs text-slate-450">({doc.reviewCount})</span>
                      </div>
                    </td>
